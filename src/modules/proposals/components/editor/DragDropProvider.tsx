...
export const DragDropProvider: React.FC<DragDropProviderProps> = ({
  ...
  }, [containers, updateItem, snapPosition]);

  const alignItems = useCallback((itemIds: string[], alignment: 'left' | 'center' | 'right' | 'top' | 'middle' | 'bottom') => {
    if (itemIds.length < 2) return;

    const items = itemIds.map(findItemById).filter(Boolean) as DragDropItem[];
    ...
    switch (alignment) {
      case 'left':
        ...
        items.forEach(item => {
          if (item.position) {
            updateItem(item.id, {
              position: snapPosition({ ...item.position, x: targetValue })
            });
          }
        });
        break;
      ...
      case 'top':
        ...
        items.forEach(item => {
          if (item.position) {
            updateItem(item.id, {
              position: snapPosition({ ...item.position, y: targetValue })
            });
          }
        });
        break;
  ...
  }, [findItemById, updateItem, snapPosition]);}]}
