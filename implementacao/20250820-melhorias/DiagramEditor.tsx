...
import { DiagramPalette } from '../../../../patches/DiagramPalette'; // Importar a paleta

// Componente de nó customizado
interface CustomNodeProps {
  data: { label: string; onAddNode?: (id: string, type: string) => void; };
  id: string;
  selected?: boolean;
  type: string;
}

const CustomNode = ({ data, id, selected, type }: CustomNodeProps) => {
  const [showAddButton, setShowAddButton] = useState(false);

  return (
    <div
      className={cn(
        "px-4 py-2 shadow-md rounded-md bg-white border-2 border-stone-400 relative",
    ...
    >
      ...
      {showAddButton && selected && data.onAddNode && (
        <Button
          variant="ghost"
          size="icon"
          className="absolute -top-2 -right-2 h-6 w-6 rounded-full bg-blue-500 text-white hover:bg-blue-600"
          onClick={() => data.onAddNode?.(id, type)}
        >
...
const InputNode = ({ data, id, selected, type }: CustomNodeProps) => {
  const [showAddButton, setShowAddButton] = useState(false);
  return (
    <div
      className={cn(
        "px-4 py-2 shadow-md rounded-md bg-green-100 border-2 border-green-400 relative",
        selected && "border-blue-500"
      )}
      onMouseEnter={() => setShowAddButton(true)}
      onMouseLeave={() => setShowAddButton(false)}
...
const OutputNode = ({ data, id, selected, type }: CustomNodeProps) => {
  const [showAddButton, setShowAddButton] = useState(false);
  return (
    <div
      className={cn(
        "px-4 py-2 shadow-md rounded-md bg-red-100 border-2 border-red-400 relative",
        selected && "border-blue-500"
      )}
      onMouseEnter={() => setShowAddButton(true)}
      onMouseLeave={() => setShowAddButton(false)}
...
export function DiagramEditor({
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      const newNode: Node = {
        ...
        type,
        position,
        data: { label: `${type} node`, onAddNode: handleAddNodeFromExisting },
      };
  ...
  const handleAddNodeFromExisting = useCallback((sourceNodeId: string, sourceType: string) => {
    const sourceNode = nodes.find(n => n.id === sourceNodeId);
    if (!sourceNode) return;

    const newNode: Node = {
      id: `node-${Date.now()}`,
      type: sourceType, // Pode ser o mesmo tipo do nó de origem ou um padrão
      position: {
        x: sourceNode.position.x + 200, // Posição relativa ao nó de origem
        y: sourceNode.position.y + Math.random() * 50 - 25,
      },
      data: { label: `Novo nó de ${sourceNode.data.label}`, onAddNode: handleAddNodeFromExisting },
    };

    setNodes((nds) => [...nds, newNode]);
    setEdges((eds) => [...eds, { id: `e${sourceNodeId}-${newNode.id}`, source: sourceNodeId, target: newNode.id, type: 'default' }]);
  ...
  const addNode = useCallback(() => {
    ...
    const newNode: Node = {
      ...
      data: { label: newNodeLabel, onAddNode: handleAddNodeFromExisting },
    };

    setNodes((nds) => [...nds, newNode]);
    setNewNodeLabel('');
  }, [newNodeLabel, setNodes, handleAddNodeFromExisting]);
  ...
  const saveDiagram = async () => {
    ...
    try {
      ...
      logError('Erro ao salvar diagrama', 'DiagramEditor', { error: error instanceof Error ? error.message : String(error) });
